with km_null_table as (
    ...
    select rdc._id_oid, distance
           ...
           where distance_type in ('osrm', 'predicted')
             and nvl(deliver_date, cancel_date) >= dateadd(hour, -24, '{query_start}')
             and nvl(deliver_date, cancel_date) < '{query_end}'
             and rdc.distance is not null
             and mo.domaintype in (4)
             {warehouse_filter}

        ) x on mo._id_oid=x._id_oid


    where (status in (900, 1000) or (courier_courier_oid IS NOT NULL and cancel_date IS NOT NULL))
     ...
     order_table as
         (select _id_oid                                                                                                                                            as order_id,
                 ...
                 lag(to_lon, 1)
                 over (partition by courier_fleetvehicle_oid,warehouse_warehouse_oid order by nvl(mo.deliver_date, mo.cancel_date) ::timestamp) as prev_lon,
                 lag(to_lat, 1)
                 over (partition by courier_fleetvehicle_oid,warehouse_warehouse_oid order by nvl(mo.deliver_date, mo.cancel_date) ::timestamp) as prev_lat,
                 ROW_NUMBER()
                 over (partition by day,courier_id order by nvl(mo.deliver_date, mo.cancel_date) ::timestamp)                                   as order_number
          from etl_market_order.marketorders mo
                                             left join etl.market_analytics.warehouse_locations wl on mo.warehouse_warehouse_oid = wl.warehouse

          where nvl(mo.deliver_date, mo.cancel_date) >= dateadd(hour, -24, '{query_start}')
     ...
     after_refill as (
         ...
         from (
                  ...
                  from (
                           ...
                           where compare_date >= '{query_start}'
                             and compare_date < dateadd(hour, 2, '{query_end}')
                             {warehouse_warehouse_filter}
                             )
                  where data_reason_oid = '5ece95a7f0cc3239856b2511'
                    and compare_date >= '{query_start}'
                    and compare_date < '{query_end}'
              )),
    ...
    where (status in (900, 1000) or (courier_courier_oid IS NOT NULL and cancel_date IS NOT NULL))
         and nvl(mo.deliver_date, mo.cancel_date) >= dateadd(hour, -24, '{query_start}')
         and nvl(mo.deliver_date, mo.cancel_date) < '{query_end}'
         and mo.domaintype in (4)
         and rdc.distance_type = 'osrm'
         group by 1
         {warehouse_filter}
     )
...
from km_null_table as nl
         ...
         left join order_table o on nl.order_id = o.order_id
         left join after_refill ar on o.order_id = ar.order_id
         left join avg_distance_table av on o.warehouse_id = av.warehouse_id
where nl.deliver_date >= '{query_start}'
  and nl.deliver_date < '{query_end}'